...
## L-06 — AC 한 줄의 비즈니스 의미가 사용자 직관과 어긋날 때, AI 는 학생의 첫 결정을 그대로 따른다

- **발견 경로**: PBI-12 머지 직전 학생이 "STUDY_WORDS 는 언제 충족되는가" 를 재확인한 시점
- **영향**: U-20 의 AC "단어 20개 학습" 은 자연어이며 정의가 명세에 없다. 학생이 사전에 결정한 정의("오늘 응시 퀴즈의 distinct word")를 AI 가 그대로 코드로 구현했으나, 실제 사용자 입장에서는 (1) TAKE_QUIZ 미션과 트리거가 동일, (2) 단어 학습 페이지 클릭이 카운트 안 됨, (3) 표준 퀴즈 한 번이면 자동 만점이라는 세 가지 라벨-동작 불일치가 발견됐다. AI 는 결정의 정합성을 학생에게 되묻지 않고 사양으로 받았다.
- **대응 패턴**: AC 가 자연어 한 줄로 표현된 미션·진척률·점수 등 **"학습 도메인의 정의 모호함"** 영역에서는 v1 프롬프트 본문에 "이 정의가 사용자 직관과 어긋나지 않는지 두세 가지 사용 시나리오를 작성한 뒤 다시 검증할 것" 을 의무화한다. 다음 PBI 의 2-A 사전 시나리오에 본 항목을 직접 인용.
- **본 PBI 의 교정**: STUDY_WORDS 의 카운트 소스를 `word_study_events` 테이블로 전환 (B-옵션). `POST /api/words/{id}/view` 신규 엔드포인트 + 단어 상세·플래시카드에서 호출. 자세한 내용은 `prompts/logs/PBI-12_daily_mission.md` §6-B 참조.
