...
export default function FlashcardPage() {
  ...
  // PBI-12 STUDY_WORDS 미션: 단어 학습 이벤트 기록
  useEffect(() => {
...
`;
const TopMeta  = styled.div`display: flex; flex-direction: column; align-items: center; gap: 3px;`;
const TopBadge = styled.span`font-size: 15px; font-weight: 700; color: ${({ $accent }) => $accent};`;
...
`;
const Front   = styled.div`display: flex; flex-direction: column; align-items: center; gap: 14px;`;
...
`;
const PartTag = styled.span`font-size: 11px; font-weight: 600; padding: 3px 11px; border-radius: 9999px; background: #FFF0DC; color: #B07040;`;
const Word = styled.h2`font-size: 44px; font-weight: 800; color: #2D1B0E; letter-spacing: -.5px; text-align: center;`;
...
`;
const Back = styled.div`display: flex; flex-direction: column; align-items: center; gap: 16px; animation: ${fadeUp} .35s ease;`;
const HDivider = styled.hr`width: 100%; border: none; border-top: 2px solid ${({ $accent }) => $accent}33; margin: 12px 0 0;`;
const Meaning = styled.p`font-size: 30px; font-weight: 800; color: #2D1B0E; text-align: center;`;
...
const Nav  = styled.div`width: 100%; display: flex; align-items: center; justify-content: space-between; margin-top: 24px;`;
...
const DoneOther      = styled.button`background: transparent; color: #B0926A; border: none; border-radius: 12px; padding: 8px 20px; font-size: 14px; font-weight: 600; cursor: pointer; text-decoration: underline; text-underline-offset: 3px; &:hover { color: #F6841F; }`;
